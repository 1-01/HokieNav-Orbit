GMAT = readtable("compare\GMAT_SIM_ISS_2021_May_20-04-06-02_UTC_(Day_140).txt");

% Interpolate Matlab results to plot residual
MATLAB.t = interp1(traj.t/86400*24*60, traj.t/86400*24*60, GMAT.HokieNav_ElapsedMinutes);
MATLAB.PX = interp1(traj.t/86400*24*60, traj.x.PX, MATLAB.t);
MATLAB.PY = interp1(traj.t/86400*24*60, traj.x.PY, MATLAB.t);
MATLAB.PZ = interp1(traj.t/86400*24*60, traj.x.PZ, MATLAB.t);
MATLAB.VX = interp1(traj.t/86400*24*60, traj.x.VX, MATLAB.t);
MATLAB.VY = interp1(traj.t/86400*24*60, traj.x.VY, MATLAB.t);
MATLAB.VZ = interp1(traj.t/86400*24*60, traj.x.VZ, MATLAB.t);
...
plot(MATLAB.t, sqrt((MATLAB.PX - GMAT.HokieNav_EarthMJ2000Eq_X).^2 + ...
                    ...
                    (MATLAB.PZ - GMAT.HokieNav_EarthMJ2000Eq_Z).^2)*1000)
hold on
...
ylabel("Velocity Residual [m/s]", 'interpreter', 'latex')
hold off

% Plot MJ2000 results from GMAT
figure
subplot(2,1,1)
plot(GMAT.HokieNav_ElapsedMinutes, [GMAT.HokieNav_EarthMJ2000Eq_X,  GMAT.HokieNav_EarthMJ2000Eq_Y,  GMAT.HokieNav_EarthMJ2000Eq_Z])
ylabel("MJ2000 Position", 'interpreter', 'latex')
hold on
subplot(2,1,2)
plot(GMAT.HokieNav_ElapsedMinutes, [GMAT.HokieNav_EarthMJ2000Eq_VX, GMAT.HokieNav_EarthMJ2000Eq_VY, GMAT.HokieNav_EarthMJ2000Eq_VZ])
xlabel("Time $t$ [min]", 'interpreter', 'latex')
...
subplot(2,1,1)
plot(traj.t/86400*24*60, [traj.x.PX, traj.x.PY, traj.x.PZ])
ylabel("J2000 Position", 'interpreter', 'latex')
hold off
subplot(2,1,2)
plot(traj.t/86400*24*60, [traj.x.VX, traj.x.VY, traj.x.VZ])
xlabel("Time $t$ [min]", 'interpreter', 'latex')
ylabel("MJ2000 Velocity", 'interpreter', 'latex')
hold off
% % Position...
