...
% Solve
[traj.t, traj.x, traj.te, traj.xe, traj.ie] = ode113(@odeval, tspan, x0, options, gravity);

%% Plot
% Obtain inertial position
[a, e, I, w, W, v] ...
     = modeq2orbital(traj.x(:,1), traj.x(:,2), traj.x(:,3), traj.x(:,4), traj.x(:,5), traj.x(:,6));
Reci = modeq2inertial(traj.x(:,1), traj.x(:,2), traj.x(:,3), traj.x(:,4), traj.x(:,5), traj.x(:,6));
% Plot over the Earth
satglobe4e, hold on
plot3(Reci(:,1), Reci(:,2), Reci(:,3), 'Color', '#A2142F')
plot3(Reci(end,1), Reci(end,2), Reci(end,3), 'w.', 'MarkerSize', 5), hold off
% Plot radius
figure, plot(traj.t/86400, sqrt(Reci(:,1).^2 + Reci(:,2).^2 + Reci(:,3).^2) - 6378.1363), axis tight
